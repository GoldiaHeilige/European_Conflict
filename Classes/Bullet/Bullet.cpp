...
Bullet* Bullet::create(Entity* entity, std::string bulletName)
{
    auto newObj = new Bullet();
    if (newObj != nullptr && newObj->init(entity, bulletName)) {
        newObj->_dmg = WeaponCtrl::getInstance()->getCurrentWeaponStat()._atk;
        newObj->autorelease();
        return newObj;
    }
...
bool Bullet::init(Entity* entity, std::string bulletName) {
    if (!Node::init()) {
        return false;
    }
    ...
    std::string bulletSpritePath = "Bullet/" + bulletName + ".png";
...
bool Bullet::onContactBegin(PhysicsContact& contact)
{
	...
	if (bullet && target) {
		...
		if (auto damageable = dynamic_cast<IDamageable*>(target)) {
			CCLOG("Enemy hit! Dealing damage: %d", _dmg);  // Use _dmg for damage
