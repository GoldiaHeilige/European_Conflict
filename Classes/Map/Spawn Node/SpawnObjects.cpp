            ...
            if (cocos2d::FileUtils::getInstance()->isFileExist(imagePath)) {
                ...
                if (_building_Industrial_Normal_Sprite != nullptr) {
                    ...
                    if (objectInfo.find("width") != objectInfo.end() && objectInfo.find("height") != objectInfo.end()) {
                        ...
                        _building_Industrial_Normal_Body->setCategoryBitmask(PhysicsCategory::STATIC_OBJECT);
                        _building_Industrial_Normal_Body->setContactTestBitmask(PhysicsCategory::BULLET);
                    ...
                    _parentNode->addChild(_building_Industrial_Normal_Sprite, 1);
                }
            }
        }
    }
            ...
            if (cocos2d::FileUtils::getInstance()->isFileExist(imagePath)) {
                ...
                if (_Elements_Industrial_Broken_Sprite != nullptr) {
                    ...
                    if (objectInfo.find("width") != objectInfo.end() && objectInfo.find("height") != objectInfo.end()) {
                        ...
                        _building_Industrial_Broken_Body->setContactTestBitmask(PhysicsCategory::ITEM | PhysicsCategory::ENEMY | PhysicsCategory::PLAYER);
                    ...
                    _parentNode->addChild(_Elements_Industrial_Broken_Sprite, 1);
                }
            }
        }
    }
...
void SpawnObjects::spawnEnemiesFromTiled(cocos2d::TMXTiledMap* map) {
    auto objectEnemyLayer = map->getObjectGroup("Enemy Spawn");
    auto _objectsEnemy = objectEnemyLayer->getObjects();

    if (_objectsEnemy.empty()) {
        CCLOG("No objects found in the object layer 'Enemy Spawn'.");
        return;
    }
    else {
        CCLOG("Number of enemy spawn points: %lu", _objectsEnemy.size());
    }

    for (auto& obj : _objectsEnemy) {
        auto objectInfo = obj.asValueMap();

        std::string enemyName = objectInfo["name"].asString();
        float x = objectInfo["x"].asFloat();
        float y = objectInfo["y"].asFloat();
        float rotation = objectInfo.find("rotation") != objectInfo.end() ? objectInfo["rotation"].asFloat() : 0.0f;

        EntityInfo* info = nullptr;
        EntityStat* stat = nullptr;

        if (EnemyType::getInstance()->getEnemyData(enemyName, info, stat)) {
            auto enemy = Enemy::create(info, stat);

            if (enemy != nullptr) {
                enemy->setPosition(cocos2d::Vec2(x, y));
            ...
            CCLOG("Enemy with name %s not found in EnemyType data.", enemyName.c_str());
        }
    }
}
